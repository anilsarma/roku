...
def extractImages( videoFiles, directory, interval, mode=0, offset=0 ):
    """
...
"""
    ...
    for i in range(len(videoFiles)):
        videoArgs.append("-i")
        videoArgs.append( videoFiles[i])


    cmd = ["ffmpeg" ] + videoArgs + [ "-ss", "%d" % offset,
...
"""
    parser = OptionParser()
    parser.add_option( "-m", "--mode",     dest="mode", type='int',     default=0,  help="(0=SD) 4:3 1=HD 4:3 2=SD 16:9 3=HD 16:9")
    parser.add_option( "-i", "--interval", dest="interval", type='int', default=10, help="Interval between images in seconds (default is 10)")
    parser.add_option( "-o", "--offset",   dest="offset", type='int',   default=0,  help="Offset to first image in seconds (default is 7)")

    (options, args) = parser.parse_args()
    if len(args) <2:
        print "error:video files or output file not specified"
        sys.exit()


    # Get the video file to operate on
    print "length of args ...  %d "%len(args)
    videoFiles=[]
    #videoFile = args[0]
    outFile   = args[len(args)-1]
   
    for i in range(len(args)-1):
        videoFiles.append( args[i] )
    print "Creating .BIF file for ", videoFiles  
    #print videoFiles
   

    # This may be useful for determining the video format
    # Get info about the video file
    videoInfo = getMP4Info(videoFiles[0])
    if videoInfo["size"]:
    ...
    bifFile = "%s-%s.bif" % (os.path.basename(outFile).rsplit('.',1)[0], modeExtension[options.mode])
    print "output file name is ", bifFile
    # Extract jpg images from the video file
    extractImages( videoFiles, tmpDirectory, options.interval, options.mode, options.offset )
