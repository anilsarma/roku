...
Function parse_show_feed(xml As Object, feed As Object) As Void
    ...
    for each curShow in showList
        ...
        item.Genre            = validstr(curShow.genre.GetText())
        ...
        'item.StreamFormat = validstr(curShow.actors.GetText())
        ...
        item.Actors.Push(item.actors)
        item.Description = item.Synopsis
        
        srt = curShow.subtitleUrl.GetText()
        if srt <> invalid and srt <> "" then
            item.SubtitleUrl = srt
        end if
        ...
        item.Rating = validstr(curShow.rating.GetText())
         print "parseItem Rating:" ; item.Rating
         
        item.StarRating = validstr(curShow.starRating.GetText())
        if item.StarRating = "" then
            item.StarRating = "90"
        endif
        ...
        if item.element.getName() = "itemSeries" then
            item.Kids =  CreateObject("roArray", 5, true)
            parse_show_feed( curShow, item.Kids )
        endif
