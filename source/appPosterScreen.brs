    ...
    while true
        ...
        if type(msg) = "roPosterScreenEvent" then
            ...
            if msg.isListFocused() then
                ...
                print "list item selected | current show = "; m.curShow ; " element="; item.element.getName()
                if item.element.getName() = "itemSeries" then
                    displayShowSeriesScreen(item, 0)
                else
...
'**********************************************************
Function displayShowSeriesScreen(item as Object, showIndex as Integer) As Integer

   ' if validateParam(category, "roAssociativeArray", "displayShowDetailScreen") = false return -1

    'shows = getShowsForCategoryItem(item, m.curCategory)
    screen = preShowSeriesPosterScreen(item.Title, item.kids[0].Title)
    showIndex = showSeriesPosterScreen(screen, item)
